...
VAI_mount_target() {
    # Mount targetfs with Btrfs subvolumes
    mkdir -p "${target}"
    ...
    if [ -d /sys/firmware/efi ] ; then
        ...
        # Mount subvolumes
        mount -o subvol=@,compress=zstd "${disk}2" "${target}"
        mkdir -p "${target}/home"
        mount -o subvol=@home,compress=zstd "${disk}2" "${target}/home"

        # Mount EFI partition
        mkdir -p "${target}/boot/efi"
        mount "${disk}1" "${target}/boot/efi"
    else
        # BIOS system — assume single partition
        mount "${disk}1" /mnt
        btrfs subvolume create /mnt/@
        btrfs subvolume create /mnt/@home
        umount /mnt

        mount -o subvol=@,compress=zstd "${disk}1" "${target}"
        mkdir -p "${target}/home"
        mount -o subvol=@home,compress=zstd "${disk}1" "${target}/home"
    fi
}
...
VAI_configure_fstab() {
    ...
    if [ -d /sys/firmware/efi ] ; then
        ...
        echo "UUID=$uuid2 / btrfs defaults,subvol=@ 0 1" >> "${target}/etc/fstab"
        echo "UUID=$uuid2 /home btrfs defaults,subvol=@home 0 2" >> "${target}/etc/fstab"
    else
        echo "UUID=$uuid1 / btrfs defaults,subvol=@ 0 1" >> "${target}/etc/fstab"
        echo "UUID=$uuid1 /home btrfs defaults,subvol=@home 0 2" >> "${target}/etc/fstab"
    fi
}
